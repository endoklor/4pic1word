...
Изображения (опционально):
  ...
  …
  images/level20/img1.png … img4.png
...
# ──────────────────────────────────────────────────────────────
#  УРОВНИ  (1–10 оригинальные, 11–20 новые)
# ──────────────────────────────────────────────────────────────
LEVELS = [
    # ── оригинальные уровни 1–10 ──
    {"word": "ТОЧКА",    "hint": ""},
    {"word": "ВЫБОР",    "hint": ""},
    {"word": "ЛИМИТ",    "hint": ""},
    {"word": "ПЛАН",     "hint": ""},
...
def load_images(level_idx, img_size):
    ...
    imgs = []
    for i in range(1, 5):
        path = os.path.join("assets", f"level{level_idx + 1}", f"img{i}.png")
        loaded = False
...
class MenuScreen(Screen):
    ...
    def draw(self, surf):
        ...
        note = g.f_tiny.render("20 уровней  ·  подсказки  ·  система очков", True, C["text_dim"])
...
class GameScreen(Screen):
    ...
    def _hint(self):
        """Открывает случайную ещё не раскрытую букву слова."""
        if self.score < 20:
            self.toast.show("Нужно минимум 20 очков для подсказки", C["warn"])
            return

        # Собираем индексы ещё не открытых слотов
        unrevealed = [i for i in range(len(self.word)) if i not in self.revealed]
        if not unrevealed:
            self.toast.show("Все буквы уже открыты!", C["text_dim"])
            return

        # Выбираем случайный индекс из нераскрытых
        ...
        for pi, p in enumerate(self.pool):
            if p["char"] == ch and not p["used"]:
                ...
                break
        if not placed:
            # добавляем букву в пул, если подходящей не нашлось
